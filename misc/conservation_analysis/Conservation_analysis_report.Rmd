...
require("knitr")
opts_knit$set(root.dir = "~/RSeq-supplemental-analysis/misc/conservation_analysis")
getwd()
```
...
```{r,out.width=50%}
rl_cons <- rl_cons[which(rl_cons$pct_cons !=  0)]

returnTagMatrix2 <- function(lbound,ubound, rLoopRanges){
  #txdb<-hg38 required
...
returnTagMatrix2(lowerbound,upperbound,rl_cons)
```
...
Applying a uniform transformation to rl_cons:
To the percent score of rl_cons I apply a log, cubed root, and squared root transformation and then generate the resulting Q-Q plot and histogram.
```{r}
preT<-rl_cons$pct_cons
...
plots <- function(data){
  ...
  #log
  show(hist(log(data)))
  qqnorm(log(data), pch = 1, frame = FALSE)
  qqline(log(data), col = "steelblue", lwd = 2)
  #shapiro.test(subsampleGRanges(log(data), n=5000))
  
  #cubed root
  hist(sign(data) * abs(data)^(1/3))
  qqnorm(sign(data) * abs(data)^(1/3), pch = 1, frame = FALSE)
  qqline(sign(data) * abs(data)^(1/3), col = "steelblue", lwd = 2)
  #shapiro.test(subsampleGRanges(sign(data) * abs(data)^(1/3)), n=5000)
  
  #square root 
  hist(sqrt(data))
  qqnorm(sqrt(data), pch = 1, frame = FALSE)
  qqline(sqrt(data), col = "steelblue", lwd = 2)
  #shapiro.test(subsampleGRanges(sqrt(data)), n=5000)
  }


plots(preT)
#chose log transformation 


```
The log transformation appears to perform the best normalization of the percent score. Below, I apply the log transformation and generate metaplots, each with an equal number of windows. The windows were sorted according to the transformed percent score prior to plotting. 

```{r}
#apply log transformation to rl_cons
rl_cons$normal <- log(rl_cons$pct_cons)

#transform into a dataframe then arrange the windows in ascending order of log score 
rl_consDf <- as.data.frame(rl_cons)
rl_consDf <- as.data.frame(rl_consDf%>%arrange(normal))
rl_consOrderedNorm<- makeGRangesFromDataFrame(rl_consDf, keep.extra.columns = TRUE)

#get denominator to subset rl_cons by
lim <- nrow(rl_consDf)
...
#loop metaplots
for(i in c(1:10)){
  current<-rl_consOrderedNorm[c(low:high)]
  tm <- getTagMatrix(current,
                     windows = promoter
                     )
  
  print(plotAvgProf(tm,
            xlim = c(-3000,3000),
              xlab = paste(low,"-",low+denominator,sep="")))
  low<-low+denominator
  high<- high+denominator
}

```
The same trend shown by the first group of metaplots is true for the transformed data as well; lower percent scoring windows tend to be further from the TSS compared to higher percenting scoring windows. 
...
Pathway enrichment for the same 10 increment groups are carried out below: 
...
For each data table I have organized the pathways in descending order of count. 
  For groups of percent scores ranging from 0-20, most windows are implicated in the nervous system, particularly in membrane protiens involved with cell to cell neuron signaling. 
	Signaling by Rho GTPases is a common pathway amongst groups with percent scores between 20 - 70%. This family of protiens are highly conserved amongst eukaryotes and so it may not be a coincidence that it is a pathway for many windows [2].
	Another common pathway is the M phase - when a cell undergoes division. I am not sure how to
interpret this pathways prominence through out 20-60 percent scoring groups. A growing body of research claim that Rloop formation is a barrier in cell replication and can lead to damage in the cells DNA [3] and so the prevelance of the M phase pathway may support this. Perhaps because these M Phase pathways are not nessecary during a cell's Interphase, these regions will form exons of overlapping genes and are therefore Rloops at this point in the cell cycle. However, the Mphase pathway is less prevelant in higher scoring windows, and completely absent in windows with percent scores ranging from 70-100%.

As the percent score increases, pathways which are involved transcription and translation become more commmon. This would make sensegiven that Rloops are know to form during transcription [1]. 
...
gene_table<- function(lbound,ubound, rLoopRanges){
  ...
  currentCaption <- paste(lbound,"-",ubound,"% score",sep= "")
  currentData <- datatable(subset(as.data.frame(rl_limited),select = -c(width,strand,score,peak)),
  ...
  if(!file.exists(current_file)){
    export.bed(rl_limited,current_file)
  }
  if(!file.exists(current_file_csv)){
    write.csv(as.data.frame(rl_limited), current_file_csv)
  }
  show(currentData)
