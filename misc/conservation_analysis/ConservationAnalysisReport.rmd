...
```{r, echo=FALSE, out.width="50%", message=FALSE, include=TRUE, results='hide'}
...
```{r,  fig.show="hold", message=FALSE, collapse=TRUE}
...
returnTagMatrix<-function(peaks){
  #function takes a GRanges list and plots distribution of each GRanges object around a TSS
...
# Percent metaplots {.tabset}
```{r, echo=FALSE, results='asis', cache=FALSE}
levelsMetaplots <- lapply(names(rl_list), function(groupNow){
  setNow <- rl_list[[groupNow]]
  knitr::knit_child('general_tab_template.Rmd', envir = environment(), quiet = TRUE)
...
test<-splitVia(rankWindows(rl_cons),10,"strata")

#generate metaplots
#lapply(test[1:10],returnTagMatrix)

names(test) <- c("10-11% score",
                 "11-13% score",
                 "13-14% score",
                 "14-17% score",
                 "17-20% score",
...
strataMetaplots <- lapply(names(test)[1:10], function(groupNow){
  setNow <- test[[groupNow]]
  knitr::knit_child('general_tab_template.Rmd', envir = environment(), quiet = TRUE)
...
```{r,echo=FALSE, fig.show="hold", message=FALSE, warning=FALSE, cache=TRUE}
#library(regioneR)
...
perm_test(rl_cons,2,"hg38", hg38_rlfs,chrom_sizes_hg38) 
...
```{r,echo=FALSE, fig.show="hold", out.width="50%", message=FALSE, warning=FALSE,cache=TRUE, include=TRUE, results='hide'}
lapply(test[1:10], perm_test, cores=2, genome="hg38", rlfs=hg38_rlfs, chrom_sizes=chrom_sizes_hg38)
...
This could be due to the large size of the windows. The remaining 4 strata, with the last one in particular begin to show a notable peak in the local z-score, which may indicate a more legitimate relationship between these windows and the set of RLFRs.
...
enrichLinks0 <- lapply(names(dataLst0), function(group) {
...
names(enrichLinks0) <- names(dataLst0)
...
res0 <- lapply(names(dataLst0), function(groupNow) {
  ...
  permalinkNow <- enrichLinks0[[groupNow]]$shortId
  knitr::knit_child(
    'tab_template.Rmd', envir = environment(), quiet = TRUE
  )
})
cat(unlist(res0), sep = '\n')
```
...
Using the percent score as a metric for what is a highly conserved R loop proves to be an effective tool which aligns with what we currently know about r-loops. Windows with higher percent scores are more likely to share characteristics which are expected of r-loop regions. High percent scoring windows are more likely to be closer to the TSS of gene and still have a notable presence surrounding it. Conversely, lower percent scoring windows have little presence near the TSS.
...
From here there are several avenues on which to continue this sort of work. Some next steps might be to analyze  how r-loops are conserved among different species. Another might be to analyze the correlation between conserved genes and r-loops which may form from them - this idea comes the the presence of the Rho GTPase pathway in many percent score groups. 
  ...
  Finally, one could look at the difference in CG content between lesser and highly conserved r-loops. R-loops are found to form over unmethylated CpG islands [4] (which are characteristic of euchromatic regions) and so perhaps this may be another characteristic of highly conserved r-loops.
