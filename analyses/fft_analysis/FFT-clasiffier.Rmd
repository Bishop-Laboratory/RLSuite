...
output:
  html_document:
    ...
    theme: cerulean
params:
  datadir: 'datasets_for_fft_testing'
  seed: 100111001
---
...
getZscores <- function(fname){
  ...
  Zscores <- c()
  #get path the file
  fpath <- file.path(params$datadir, "rda_qc", fname)
  #assert file exists
  if(file.exists(fpath)){
    ...
    try(Zscores <- data_list$rlfs_data[[2]]$`regioneR::numOverlaps`$shifted.z.scores)
  }
  #return Zscores
  return(Zscores)
}
...
Zdata <- data.frame(row.names = metadata$sample_name)
...
indexes = createDataPartition(Zdata$group, p = .60, list = F)
train = Zdata[indexes, ]
disco = Zdata[-indexes, ]
...
yhat <- predict(stacked_model, disco)
confusionMatrix(yhat, disco$group)
```