...
if (sys.nframe() == 0L) {
  ...
  PRI <- "analyses/Prepare-RMapDB-Tables/GeneExpSamples/rlorig_pri.rda"
  ...
  genexp_2_rmap <- full_join(rmapraw,
    ...
    fill(gene_exp_sample_id, .direction = "up") %>%
    fill(rmap_sample_id, .direction = "down") %>%
